...
function scr_cardplay_deck_init(deck, character_id){
	var starting_deck = ds_map_find_value(global.cfg_character_starting_decks, character_id.character_type);
	for(var i = 0; i < array_length(starting_deck); ++i){
		var card = instance_create_layer(deck.x,deck.y,"Instances", starting_deck[i]);
		card.owner = character_id;
		scr_card_hide(card);
